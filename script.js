...
class StoryScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.dialogues = [
            ...
            { name: this.charData.name, text: "적들의 행방은 파악되었습니까?" },
            { name: "HOKAGE", text: "북쪽 안개 계곡으로 향했다는 첩보가 들어왔다. 그곳은 지형이 험하고 적들의 매복이 예상된다." },
            { name: this.charData.name, text: "걱정 마십시오. 제 목숨을 걸고 반드시 되찾아 오겠습니다!" },
            { name: "HOKAGE", text: "서둘러라! 그들이 국경의 결계를 넘기 전에 막아야 한다. 마을의 운명이 네 손에 달렸다!" },
            { name: "SYSTEM", text: "(안개 계곡으로의 추격전이 시작됩니다...)" }
        ];
        this.currentLine = 0; this.currentChar = 0; this.isTyping = false;
    ...
    advanceDialogue() {
        if (this.isTyping) {
            this.isTyping = false; if(this.typeTimer) this.typeTimer.remove();
            ...
            } else this.showNextLine();
        }
    }
    showNextLine() {
        this.isTyping = true; this.currentChar = 0;
        this.nameText.setText(this.dialogues[this.currentLine].name); this.dialogueText.setText('');
        const fullText = this.dialogues[this.currentLine].text;
        this.typeTimer = this.time.addEvent({
            delay: 40, callback: () => {
                this.currentChar++; this.dialogueText.setText(fullText.slice(0, this.currentChar));
...
class GameScene extends Phaser.Scene {
    ...
    updateHPBar() {
        this.hpBar.clear(); this.hpBar.fillStyle(0x000000, 0.6).fillRect(20, 60, 204, 24);
    ...
    handleMovement(delta) {
        const speed = 400; if (this.isRoping) return;
        ...
        if (Phaser.Input.Keyboard.JustDown(this.cursors.up) && this.player.body.touching.down) {
            this.player.setVelocityY(-950); JuiceManager.emitParticles(this, this.player.x, this.player.y+20, 'DUST');
        }
        ...
        if (Phaser.Input.Keyboard.JustDown(this.keys.SPACE)) {
            if (this.isRoping) { this.isRoping = false; this.player.body.setAllowGravity(true); }
                ...
                if (Phaser.Math.Distance.BetweenPoints(this.player, nearest) < 400) {
                    this.isRoping = true; this.ropeTarget = nearest; this.player.body.setAllowGravity(false);
                }
            }
        }
        if (this.isRoping) {
            ...
            this.ropeLine.lineStyle(2, 0x8b4513, 1).beginPath().moveTo(this.ropeTarget.x, this.ropeTarget.y).lineTo(this.player.x, this.player.y).strokePath();
        }
    }
    ...
    handleCloud(p, c) {
        if (!c.isPressed) {
            c.isPressed = true;
            this.tweens.add({ targets: c, y: c.y + 5, duration: 100, yoyo: true, onComplete: () => { c.isPressed = false; } });
        }
    }
    ...
    spawnEnemy() { this.enemies.create(this.player.x + 800, 400, 'e1').setVelocityX(-200); }
